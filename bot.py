...
REQUIRED_CHANNEL = "https://t.me/GenerateAll"
PORT = int(os.environ.get("PORT", 5000)) # المنفذ الذي سيعمل عليه الموقع
...
# دالة لتشغيل السيرفر في خيط منفصل (Thread) لكي لا يعطل البوت
def run_flask():
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args and str(args[0]).isdigit():
        referrer = int(args[0])
        if referrer != user_id:
            db.process_referral(referrer, user_id)
            try:
                await context.bot.send_message(referrer, f"شكراً! قمت بدعوة مستخدم جديد وحصلت على {SHARE_POINTS} نقطة.")
            except: pass
    ...
    keyboard = [[InlineKeyboardButton("🚀 فتح التطبيق", web_app={'url': f'https://your-render-url.com/index.html?user_id={user_id}'})]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text("مرحباً بك في بوت التوليد!", reply_markup=reply_markup)
